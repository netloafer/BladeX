...
The `bladex_*` tool definitions are already in your tool list; their
...
| `Error: edit 3 of 4 failed (...); NOTHING was written` | A batch is all-or-nothing. **None** of the edits landed and the ledger is unchanged. Fix the one named and resend the *whole* batch, not just that one. |
| `Error: match '...' matches 2 entries (...)` | Ambiguous. The message lists the candidates; make `match` longer, or pass `index`. |
| `Error: match '...' matches entries in 2 sections (...)` | You removed by `match` without a `section` and it hit more than one section. The message names them; pass `section`, or make `match` longer. |
| `Error: remove requires match or index — got keys [...]` | The edit says `remove` but never says *what* to remove. The keys you sent are listed; the entry's text goes in `match`, not `text`. |
<!-- /bladex:ledger -->
...
A round trip is not cheap: everything in this conversation is sent upstream
again for each one.

<!-- bladex:ledger -->
Batch several edits in one `bladex_ledger_update` call (`entries`). Finishing a
step is normally two edits -- add to `verified`, remove the `next` entry it
resolves -- and those belong in the same call, not two. Either all of the edits
apply or none does, so you never end up with half a ledger. Prefer `match` (the
entry's text, or a unique prefix of it) over `index`: positions shift when
someone else writes to the ledger, text does not.

When you remove by `match`, `section` is optional -- the text already identifies
the entry, so BladeX looks through the whole ledger. Pass `section` only if the
same text appears in more than one of them. Removing by `index` still needs
`section`: an index is a position *within* one section.

When you also have your own tool calls to make this turn, issue the ledger
update **in the same turn** rather than alone -- a ledger-only turn costs a full
extra model round.
<!-- /bladex:ledger -->
